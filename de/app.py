...
class HashingApp(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        self.title("Datei-Hashing")
        self.geometry("600x420")
        self.configure(bg="#23272e")

        self.label_info = tk.Label(self, text="Datei per Drag & Drop hier ablegen", bg="#23272e", fg="#56c7ff", font=("Segoe UI", 16, "bold"))
        self.label_info.pack(pady=25)

        self.drop_area = tk.Label(self, text="🗂️ Datei hierhin ziehen", bg="#303542", fg="#c1c1c1", width=60, height=6, font=("Segoe UI", 13, "bold"), relief="groove", borderwidth=3)
    ...
    def handle_drop(self, event):
        file_path = event.data.strip("{}")  # strip curly brackets for Windows paths
        if not os.path.isfile(file_path):
            self.label_info.config(text="Ungültige Datei!", fg="#ff5151")
            return
...
if __name__ == "__main__":
    try:
        ...
        print("Bitte führen Sie zuerst aus: pip install tkinterdnd2")
        exit(1)
